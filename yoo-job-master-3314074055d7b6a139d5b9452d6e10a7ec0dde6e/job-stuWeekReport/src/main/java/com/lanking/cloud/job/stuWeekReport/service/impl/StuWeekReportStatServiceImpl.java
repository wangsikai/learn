...
import com.lanking.cloud.job.stuWeekReport.dao.StudentHomeworkDAO;
import com.lanking.cloud.job.stuWeekReport.dao.StudentWeekReportDAO;
import com.lanking.cloud.job.stuWeekReport.dao.UserDAO;
...
@Service
public class StuWeekReportStatServiceImpl implements StuWeekReportStatService {
	...
	public void stat(Long userId) {
		...
		try {
			...
			stuWeekReport.setCreateAt(new Date());
	...
	@Transactional
	@Override
	public void statStuClassWeekReport(Long classId) {
		SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
		try {
			Calendar cal = Calendar.getInstance();
			// 临时处理给测试测
			...
			if (CollectionUtils.isNotEmpty(list)) {
				...
				for (Map pa : list) {
					...
					stuClassWeekReport.setCreateAt(new Date());
	...
	public CursorPage<Long, Map> queryUserId(CursorPageable<Long> pageable, int modVal) {
		return userDAO.queryUserId(pageable, modVal);
	}

	@Override
	public CursorPage<Long, Map> queryClassIds(CursorPageable<Long> pageable, int modVal) {
		return homeworkClazzDAO.queryClassIds(pageable, modVal);
	}
