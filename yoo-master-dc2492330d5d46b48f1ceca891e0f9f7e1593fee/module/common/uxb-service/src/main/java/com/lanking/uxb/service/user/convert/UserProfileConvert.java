...
import com.lanking.uxb.service.user.value.VParentProfile;
import com.lanking.uxb.service.user.value.VStudent;
import com.lanking.uxb.service.user.value.VStudentProfile;
import com.lanking.uxb.service.user.value.VTeacher;
import com.lanking.uxb.service.user.value.VTeacherProfile;
...
@Component
public class UserProfileConvert extends Converter<VUserProfile, User, Long> {
	...
	@Override
	protected VUserProfile convert(User s) {
		VUserProfile v = new VUserProfile();
		v.setId(s.getId());
	...
	@Override
	public VUserProfile get(Long id) {
		User user = new User();
		user.setId(id);
		return this.to(user);
	}

	public VUserProfile get(Long id, UserConvertOption option) {
		User user = new User();
		user.setId(id);
	...
	@Override
	public Map<Long, VUserProfile> mget(Collection<Long> ids) {
	...
	public Map<Long, VUserProfile> mget(Collection<Long> ids, UserConvertOption option) {
		Map<Long, User> sMap = Maps.newHashMap();
		for (Long id : ids) {
			User user = new User();
			user.setId(id);
	...
	protected void initAssemblers(List<ConverterAssembler> assemblers) {
		// 用户的总体信息
		assemblers.add(new ConverterAssembler<VUserProfile, User, Long, UserInfo>() {
			@Override
			...
			@Override
			public Long getKey(User s, VUserProfile d) {
				return s.getId();
			}

			@Override
			public void setValue(User s, VUserProfile d, UserInfo value) {
				// 基础公共信息
				...
				d.setSex(value.getSex() == null ? Sex.UNKNOWN : value.getSex());
				...
				if (value.getUserType() == UserType.TEACHER) {// 老师
					...
					VTeacherProfile tp = new VTeacherProfile();
					tp.setSchoolName(validBlank(teacher.getSchoolName()));
					if (teacher.getWorkAt() != null) {
						tp.setTeachYear(TeachUtil.getTeachYear(teacher.getWorkAt()));
						tp.setWorkAt(teacher.getWorkAt());
					}
					// profile的额外信息
					...
					VStudentProfile sp = new VStudentProfile();
					sp.setYear(student.getYear());
					...
					VParentProfile pp = new VParentProfile();
					d.setPp(pp);
					d.setP(pp);
				}
		...
		// profile特有,账户信息
		assemblers.add(new ConverterAssembler<VUserProfile, User, Long, Account>() {
			@Override
			public boolean accept(User s) {
				return true;
			}
			...
			@Override
			public void setValue(User s, VUserProfile d, Account value) {
				d.setAccount(accountConvert.to(value));
			}
		...
		// 教师的学科信息
		assemblers.add(new ConverterAssembler<VUserProfile, User, Integer, VSubject>() {
			@Override
			public boolean accept(User s) {
			...
			@Override
			public Integer getKey(User s, VUserProfile d) {
			...
			@Override
			public void setValue(User s, VUserProfile d, VSubject value) {
				if (value != null && s.getUserInfo().getUserType() == UserType.TEACHER) {
					VTeacherProfile vt = d.getTp();
					vt.setSubject(value);
					d.setT(vt);
				}
			}
		...
		// 用户的统计信息
		assemblers.add(new ConverterAssembler<VUserProfile, User, Long, VUserState>() {
			@Override
			public boolean accept(User s) {
			...
			@Override
			public void setValue(User s, VUserProfile d, VUserState value) {
		...
		assemblers.add(new ConverterAssembler<VUser, User, Integer, VTextbook>() {
			...
			public Integer getKey(User s, VUser d) {
				...
				if (s.getUserInfo().getUserType() == UserType.TEACHER) {
					return ((Teacher) s.getUserInfo()).getTextbookCode();
				}
				return null;
			}
