...
@Component
public class ZyStudentFallibleQuestionConvert
		...
		if (CollectionUtils.isNotEmpty(s.getOcrHisAnswerImgs())) {
			...
			for (Map a : s.getOcrHisAnswerImgs()) {
				...
				if (a.get("imageIds") != null) {
					List<Object> imageIds = (List<Object>) a.get("imageIds");
					logger.error("0123456789 000 imageIds " + imageIds);
					for (Object id : imageIds) {
						imgUrls.add(FileUtil.getUrl(Long.parseLong(id.toString())));
					}
				} else {
