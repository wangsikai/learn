...
/**
 * Section -> VStudentExerciseConvert
 *
 * @author xinyu.zhou
 * @since yoomath V1.6
 */
@Component
public class ZyStudentExerciseSectionConvert extends Converter<VStudentExerciseSection, VSection, Long> {
			...
			for (StudentExerciseSection ses : ret) {
				...
				if (lastMonth == null) {
					...
					if (ses.getLastMonthDoCount() > MIN_DO_COUNT) {
						...
						complete = ((ses.getLastMonthDoCount() - ses.getLastMonthWrongCount()) * 100) / MIN_DO_COUNT;
					}
				}

				totalComplete += complete;
				v.setDoCount(ses.getDoCount() + v.getDoCount());
			}
