 ...
 */
@RestController
@RequestMapping("account/2/eduyun")
public class Register2EduYunController {
	...
	public Value checkVerifyCode(String mobile, String email, String verifyCode, UserType userType, Product source) {
		...
		if (account != null && credentialValue != null) {
			...
			if (credentialType == CredentialType.EDUYUN && null != credential) {
				YunUser yunUser = Security.getSession().getAttrSession().getObject("yunuser", YunUser.class);
				if (!yunUser.getPersonid().equals(credential.getUid())) {
					// 不允许多个教育云用户，绑定同一个本地账户
					map.put("flag", 2);
				}
			} else if (credentialType == CredentialType.EDUYUN && null == credential) {
				// 教育云教师用户不允许绑定到学生账户上
	...
	public Value register(RegisterForm form, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			if (form.getType() == UserType.TEACHER) {
				...
				// }
				if (form.getPhaseCode() == 2) {
					form.setSubjectCode(202);
				} else if (form.getPhaseCode() == 3) {
	...
	private void processClazz(User user) {
		...
		List<YunClass> scclazzes = yunUser.getTeachesubjectlist();
		List<String> codes = new ArrayList<String>();
		if (scclazzes.size() == 0) {
			return;
		}

		for (YunClass clazz : scclazzes) {
			codes.add(clazz.getClassid());
		}

		// 带第三方及教师组合key的班级列表
		Map<String, List<HomeworkClazz>> homeworkClazzMap = homeworkClassService.findTeaUsedByFromCode(
				ClazzFrom.EDUYUN, codes);

		if (scclazzes.get(0) == null) {
			return;
		}
		if (yunUser.getUsertype() == YunUserType.STUDENT.getValue()) {
			// 学生
			for (YunClass clazz : scclazzes) {
		...
		} else if (yunUser.getUsertype() == YunUserType.TEACHER.getValue()) {
			// 老师
			for (YunClass clazz : scclazzes) {
				List<HomeworkClazz> homeworkClazzs = homeworkClazzMap.get(clazz.getClassid()); // 相同第三方班级码的班级集合
