...
public class ScanPayBusiness {
	...
	public ScanPayBusiness(Configure configure) throws IllegalAccessException, ClassNotFoundException,
	...
	public void run(ScanPayReqData scanPayReqData, ResultListener resultListener) throws Exception {
		...
		log.i("支付API返回的数据如下：");
		payServiceResponseString = scanPayService.request(scanPayReqData, configure);
		...
		ReportReqData reportReqData = new ReportReqData(configure,
				scanPayReqData.getDevice_info(),
				APIConfigure.PAY_API,
	...
	private boolean doOnePayQuery(String outTradeNo) throws Exception {
		...
		payQueryServiceResponseString = scanPayQueryService.request(scanPayQueryReqData, configure);
	...
	private boolean doOneReverse(String outTradeNo) throws Exception {
		...
		reverseResponseString = reverseService.request(reverseReqData, configure);
