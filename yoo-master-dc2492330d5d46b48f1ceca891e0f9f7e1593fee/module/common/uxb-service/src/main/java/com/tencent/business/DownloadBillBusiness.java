...
public class DownloadBillBusiness {
	...
	public DownloadBillBusiness(Configure configure) throws IllegalAccessException, ClassNotFoundException,
	...
	public void run(DownloadBillReqData downloadBillReqData, ResultListener resultListener) throws Exception {
		...
		log.i("对账单API返回的数据如下：");
		downloadBillServiceResponseString = downloadBillService.request(downloadBillReqData, configure);
		...
		try {
			...
			ReportReqData reportReqData = new ReportReqData(configure, downloadBillReqData.getDevice_info(),
					APIConfigure.DOWNLOAD_BILL_API, (int) (totalTimeCost),// 本次请求耗时
