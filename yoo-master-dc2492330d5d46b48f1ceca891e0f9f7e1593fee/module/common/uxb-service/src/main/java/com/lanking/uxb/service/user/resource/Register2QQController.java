 ...
 */
@RestController
@RequestMapping("account/2/qq")
public class Register2QQController {
	...
	public Value checkVerifyCode(String mobile, String email, String verifyCode, UserType userType, Product source) {
		...
		if (account != null && credentialValue != null) {
			...
			if (credentialType == CredentialType.QQ && null != credential) {
				QQUser qquser = Security.getSession().getAttrSession().getObject("qquser", QQUser.class);
				if (!qquser.getPersonid().equals(credential.getUid())) {
					// 不允许多个QQ用户，绑定同一个本地账户
					map.put("flag", 2);
				}
			}
