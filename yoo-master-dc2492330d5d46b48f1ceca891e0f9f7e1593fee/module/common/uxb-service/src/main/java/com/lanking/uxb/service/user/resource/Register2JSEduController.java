 ...
 */
@RestController
@RequestMapping("account/2/jsedu")
public class Register2JSEduController {
	...
	public Value checkVerifyCode(String mobile, String email, String verifyCode, UserType userType, Product source) {
		...
		if (account != null && credentialValue != null) {
			...
			if (credentialType == CredentialType.JSEDU && null != credential) {
				JSEduUser jsUser = Security.getSession().getAttrSession().getObject("jsuser", JSEduUser.class);
				if (!jsUser.getIdentityId().equals(credential.getUid())) {
					// 不允许多个江苏电信教育平台用户，绑定同一个本地账户
					map.put("flag", 2);
				}
			} else if (credentialType == CredentialType.JSEDU && null == credential) {
				// 江苏电信教育教师用户不允许绑定到学生账户上
	...
	private void processClazz(User user) {
		...
		List<JSEduTeacherClass> scclazzes = null;
		List<String> codes = new ArrayList<String>();
		if ("5".equals(jsUser.getIdentityType())) {
			JSEduStudent student = jsUser.getStudentinfo();
			JSEduTeacherClass teaClass = new JSEduTeacherClass();
			teaClass.setClassId(student.getClassId());
			teaClass.setClassName(student.getClassName());
			teaClass.setSchId(student.getSchId());
			teaClass.setSchName(student.getSchName());
			scclazzes = new ArrayList<JSEduTeacherClass>();
			scclazzes.add(teaClass);
		} else if ("1".equals(jsUser.getIdentityType())) {
			scclazzes = jsUser.getTeacherClasses();
		}
		...
		for (JSEduTeacherClass clazz : scclazzes) {
		...
		if ("5".equals(jsUser.getIdentityType())) {
			...
			// continue;
			// }
			// List<HomeworkClazz> homeworkClazzs =
			// homeworkClazzMap.get(clazz.getClassId()); // 相同第三方班级码的班级集合
			// HomeworkClazz homeworkClazz = null;
			// if (null != homeworkClazzs) {
			// for (HomeworkClazz hc : homeworkClazzs) {
			// if (hc.getTeacherId() != null && hc.getTeacherId() ==
			// user.getId().longValue()) {
			// homeworkClazz = hc;
			// } else if (hc.getTeacherId() == null) {
			// // 有无教师的班级，更新教师
			// homeworkClassService.updateTeacherByFromCode(ClazzFrom.SCEDU,
			// clazz.getClassId(),
			// user.getId());
			// homeworkClazz = hc;
			// }
			...
			// // 没有该老师的班级，创建班级
			// homeworkClazz =
			// homeworkClassService.createByThird(clazz.getClassname(),
			// user.getId(),
			// ClazzFrom.SCEDU, clazz.getClassId());
			// if (null != homeworkClazzs && homeworkClazzs.size() > 0) {
			// // 将其他不是该老师班级下的学生，加入到新班级中
			// List<Long> studentIds =
			// homeworkStudentClazzService.listClassStudents(homeworkClazzs.get(0)
			// .getId());
			// for (Long studentId : studentIds) {
			// homeworkStudentClazzService.join(homeworkClazz.getId(),
			// studentId);
			// }
			// }
			// }
