...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:mml="http://www.w3.org/1998/Math/MathML"
			...
			<xsl:element name="w:rPr">
				<xsl:element name="w:rFonts">
					<xsl:attribute name="w:ascii">
						<xsl:value-of select="'Cambria Math'" />
					</xsl:attribute>
  ...
  <xsl:template match="mml:munder">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <xsl:choose>
          <!-- modify 2016.6.12 update 原$fIsBar=1 处理下划线 -->
  ...
  <xsl:template match="mml:mtable">
    ...
    <xsl:choose>
     <!-- modify 原$fEqArray=1，改为9处理集合括号内的公式不居左的问题 -->
      <xsl:when test="$fEqArray=9">
            ...
            <m:mcs>
              <m:mc>
                <m:mcPr>
				  ...
				  <!-- modify 2016-06-02 update 原center -->
                  <m:mcJc m:val="left" />
                </m:mcPr>
              </m:mc>
            </m:mcs>
