...
class CacheCounterServiceImpl implements CounterService {
	...
	public Counter getCounter(Biz biz, long bizId) {
		Counter counter = counterCacheService.get(biz, bizId);
		if (counter == null) {
			counter = counterRepo.find("$getCounter", Params.param("biz", biz.getValue()).put("bizId", bizId)).get();
			if (counter != null) {
				counterCacheService.set(counter);
			}
		}
		return counter;
	}
	...
	public Map<Long, Counter> getCounters(Biz biz, Collection<Long> bizIds) {
		...
		if (null != bizIds && bizIds.size() > 0) {
			// get from cache
			List<Counter> cacheCounters = counterCacheService.mget(biz, bizIds);
			for (Counter counter : cacheCounters) {
				counterMap.put(counter.getBizId(), counter);
			}
			List<Long> notHits = Lists.newArrayList();
			for (Long bizId : bizIds) {
				if (!counterMap.containsKey(bizId)) {
					notHits.add(bizId);
				}
			}
			if (notHits.size() > 0) {
				List<Counter> counters = counterRepo
						.find("$getCounters", Params.param("biz", biz.getValue()).put("bizIds", notHits)).list();
	...
	public CounterDetail getCounterDetail(Biz biz, long bizId, Biz otherBiz, long otherBizId) {
		CounterDetail counterDetail = counterCacheService.get(biz, bizId, otherBiz, otherBizId);
		if (counterDetail == null) {
			counterDetail = counterDetailRepo.find("$getCounterDetail", Params.param("biz", biz.getValue())
					.put("bizId", bizId).put("otherBiz", otherBiz.getValue()).put("otherBizId", otherBizId)).get();
			if (counterDetail != null) {
				counterCacheService.set(counterDetail);
			}
		}
	...
	public Map<Long, CounterDetail> getCounterDetailsByOtherBizId(Biz biz, long bizId, Biz otherBiz,
		...
		// get from cache
		List<CounterDetail> cacheCounterDetails = counterCacheService.mget(biz, bizId, otherBiz, otherBizIds);
		for (CounterDetail counterDetail : cacheCounterDetails) {
			map.put(counterDetail.getOtherBizId(), counterDetail);
		}
		List<Long> notHits = Lists.newArrayList();
		for (Long otherBizId : otherBizIds) {
			if (!map.containsKey(otherBizId)) {
				notHits.add(bizId);
			}
		}
		if (notHits.size() > 0) {
			...
			for (CounterDetail detail : details) {
				...
				counterCacheService.set(detail);
			}
		}
	...
	public Map<Long, CounterDetail> getCounterDetailsByBizId(Biz biz, Collection<Long> bizIds, Biz otherBiz,
		...
		// get from cache
		List<CounterDetail> cacheCounterDetails = counterCacheService.mget(biz, bizIds, otherBiz, otherBizId);
		for (CounterDetail counterDetail : cacheCounterDetails) {
			map.put(counterDetail.getBizId(), counterDetail);
		}
		List<Long> notHits = Lists.newArrayList();
		for (Long bizId : bizIds) {
	...
	public void counter(Biz biz, long bizId, Count count, Number c) {
		...
		counterCacheService.invalid(biz, bizId);
	}
	...
	public void counterDetail(Biz biz, long bizId, Biz otherBiz, long otherBizId, Count count, Number c) {
		...
		counterCacheService.invalid(biz, bizId, otherBiz, otherBizId);
	}
