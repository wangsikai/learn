 ...
 */
@Component
public class CollectionQuestionIndexConvert extends Converter<CollectionQuestionDoc, QuestionCollection, Long> {
	...
	@Override
	protected CollectionQuestionDoc convert(QuestionCollection s) {
		CollectionQuestionDoc v = new CollectionQuestionDoc();
		v.setId(s.getId());
		v.setQuestionId(s.getQuestionId());
		v.setTeacherId(s.getUserId());
		v.setDifficulty(BigDecimal.valueOf(s.getDifficulty() == null ? 1.0f : s.getDifficulty()));
		v.setTypeCode(s.getTypeCode());
	...
	protected void initAssemblers(List<ConverterAssembler> assemblers) {
		...
		assemblers.add(new ConverterAssembler<CollectionQuestionDoc, QuestionCollection, Long, Question>() {
			...
			@Override
			public Long getKey(QuestionCollection s, CollectionQuestionDoc d) {
				return s.getQuestionId();
			}

			@Override
			public void setValue(QuestionCollection s, CollectionQuestionDoc d, Question value) {
				if (value != null) {
		...
		// 知识点
		assemblers.add(new ConverterAssembler<CollectionQuestionDoc, QuestionCollection, Long, List<MetaKnowpoint>>() {
			...
			@Override
			public void setValue(QuestionCollection s, CollectionQuestionDoc d, List<MetaKnowpoint> value) {
		...
		// 知识点（新）
		assemblers.add(new ConverterAssembler<CollectionQuestionDoc, QuestionCollection, Long, List<KnowledgePoint>>() {
			...
			@Override
			public void setValue(QuestionCollection s, CollectionQuestionDoc d, List<KnowledgePoint> value) {
				if (CollectionUtils.isNotEmpty(value)) {
					StringBuffer buff = new StringBuffer();
		...
		// 教材章节
		assemblers.add(
				new ConverterAssembler<CollectionQuestionDoc, QuestionCollection, Long, Map<String, List<Long>>>() {
					...
					@Override
					public void setValue(QuestionCollection s, CollectionQuestionDoc d, Map<String, List<Long>> value) {
		...
		// 子题
		assemblers.add(new ConverterAssembler<CollectionQuestionDoc, QuestionCollection, Long, String>() {
			@Override
			public boolean accept(QuestionCollection s) {
				return true;
			}
			...
			@Override
			public void setValue(QuestionCollection s, CollectionQuestionDoc d, String value) {
