...
public class WebSocketServerUpstreamHandler extends NettyServerUpstreamHandler {
	...
	public WebSocketServerUpstreamHandler(LoggerFactory loggerFactory, ChannelHandler channelHandler,
			ChannelGroup channelGroup, boolean cumulative) {
	...
	private void handleHttpRequest(ChannelHandlerContext ctx, HttpRequest req) {
		...
		if (this.channelHandler != null) {
			try {
				...
				this.logger.error(e);
				this.sendHttpResponse(ctx, req, new DefaultHttpResponse(HttpVersion.HTTP_1_1, new HttpResponseStatus(
						401, e.getMessage())));
				return;
			}
		}
		...
		WebSocketServerHandshakerFactory wsFactory = new WebSocketServerHandshakerFactory(req.getUri(), subprotocols,
				allowExtensions);
	...
	protected void handleWebSocketFrame(final ChannelHandlerContext ctx, WebSocketFrame frame) throws Exception {
		if (frame instanceof CloseWebSocketFrame) {
			this.logger.info(Text.WS_CONNECTION_CLOSED_BY, ((CloseWebSocketFrame) frame).getStatusCode(),
	...
	private void handleWebSocketFrame(final ChannelHandlerContext ctx, List<WebSocketFrame> frames) throws Exception {
