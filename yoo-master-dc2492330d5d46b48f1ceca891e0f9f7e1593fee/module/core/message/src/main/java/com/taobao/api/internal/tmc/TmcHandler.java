			...
			tmcClient.getConfirmThreadPool().submit(new ConfirmWorker(0, message) {
				public void run() {
					try {
						...
						// when it happended, should hold the thread because all
						// message can't confirm success
						holdTheThread();
						// retry
						if (this.getRetry() < 3) {
							this.incrRetry();
							...
							log.warn(String.format("confirm message fail 3 times,discard it : %s", this.getMessage()
									.getRaw()));
						}
					}
				}
	...
	public void handleMessage(final Message message, final boolean ignore) {
		while (!stopped) {
			try {
				...
				log.warn(String.format("all tmc worker threads are currently busy, waiting 50 ms, appkey:%s-group:%s",
						tmcClient.getAppKey(), tmcClient.getGroupName()));
				try {
					Thread.sleep(50L);
