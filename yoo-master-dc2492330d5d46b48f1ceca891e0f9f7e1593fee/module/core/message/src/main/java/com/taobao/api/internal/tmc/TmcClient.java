...
public class TmcClient {
	...
	class InnerClient extends MixClient {
		...
		protected Map<String, Object> createConnectHeaders() {
			...
			try {
				signHeader.put(SIGN,
						TaobaoUtils.signTopRequest(signHeader, null, this.appSecret, Constants.SIGN_METHOD_MD5));
			} catch (Exception e) {
	...
	public void setUseDefaultConfirm(boolean useDefaultConfirm) throws InterruptedException {
		if (!useDefaultConfirm && confirmThreadPool == null) {
			this.confirmThreadPool = new ThreadPoolExecutor(confirmThreadCount, confirmThreadCount, fetchPeriod * 2,
					TimeUnit.SECONDS, new ArrayBlockingQueue<Runnable>(queueSize), new NamedThreadFactory(
							"tmc-confirm-worker"), new AbortPolicy());
		}
