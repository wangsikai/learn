...
public class SecurityClient {
	...
	public String decrypt(String data, String type, String session) throws SecretException {
		...
		try {
			...
			String decryptValue = TaobaoUtils.aesDecrypt(secretDataDO.getOriginalBase64Value(),
					secretContextDO.getSecret());
	...
	/**
	 * 加密之后格式。 手机号码格式：$手机号码前3位明文$base64(encrypt(phone后8位置))$111$
	 * nick格式：~base64(encrypt(nick))~111~
	 * 
	 * @param data
	 *            明文数据
