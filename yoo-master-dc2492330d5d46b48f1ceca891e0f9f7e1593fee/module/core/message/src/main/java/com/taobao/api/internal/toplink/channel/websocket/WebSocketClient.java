...
public class WebSocketClient extends NettyClient {
	...
	public static ClientChannel connect(LoggerFactory loggerFactory, URI uri, int connectTimeoutMillis)
		...
		// connect
		Channel channel = prepareAndConnect(logger, uri, pipeline, wsHandler, uri.getScheme().equalsIgnoreCase("wss"),
				connectTimeoutMillis);
		// handshake
		try {
			WebSocketClientHandshaker handshaker = wsFactory.newHandshaker(uri, WebSocketVersion.V13, null, true,
					WebSocketClientHelper.getHeaders(uri));
