...
public class RemotingClientChannelHandler extends SimpleChannelHandler {
	...
	public ByteBuffer pending(RemotingCallback handler, short operation, HashMap<String, Object> transportHeaders,
			MethodCall methodCall) throws FormatterException {
		byte[] data = this.serializationFactory.get(handler.serializationFormat).serializeMethodCall(methodCall);
	...
	public void onMessage(ChannelContext context) {
		...
		RemotingTcpProtocolHandle protocol = msg instanceof ByteBuffer ? new RemotingTcpProtocolHandle((ByteBuffer) msg)
		...
		Object flag;
		if (transportHeaders == null || (flag = transportHeaders.get(RemotingTransportHeader.Flag)) == null)
			return;
		...
		if (statusCode != null && Integer.parseInt(statusCode.toString()) > 0) {
			callback.onException(new Exception(String.format(Text.RPC_RETURN_ERROR, statusCode, statusPhrase)));
			return;
		}

		MethodReturn methodReturn = null;
		try {
			methodReturn = this.serializationFactory.get(callback.serializationFormat).deserializeMethodReturn(
					protocol.ReadContent(), callback.returnType);
