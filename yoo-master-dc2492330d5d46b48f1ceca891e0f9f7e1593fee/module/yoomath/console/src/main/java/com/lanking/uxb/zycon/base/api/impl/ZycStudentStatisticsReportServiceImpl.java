...
@Service
public class ZycStudentStatisticsReportServiceImpl implements ZycStudentStatisticsReportService {

	@Autowired
	private ZycStudentHomeworkService studenthomeworkService;
	@Autowired
	private ZycHomeworkStudentClazzService stuClazzService;
	@Autowired
	...
	public void create(int year, int month, long clazzId) {
		...
		if (homeworkClazz.getTeacherId() != null) {
			...
			if (teacher.getTextbookCode() == null || teacher.getSubjectCode() == null) {
				return;
			}
	...
	public StudentStatisticsReport createStudentStat(long stuId, long clazzId, int year, int month, int stuNum,
		...
		List<VSection> sections = sectionConvert.to(sectionService.findByTextbookCode(Long.valueOf(textbookCode)
				.intValue()));
		List<CStudentExerciseSection> vs = studentExerciseSectionConvert.to(sections);
