...
@Service
public class TaskActivityExerciseQuestionServiceImpl implements TaskActivityExerciseQuestionService {
	...
	public List<Long> getQuestion(long exerciseId) throws ExerciseException {
		return exerciseQuestionRepo.find("$TaskGetQuestion", Params.param("exerciseId", exerciseId)).list(Long.class);
	}
	...
	public ExerciseQuestion appendQuestion(long exerciseId, long questionId, Integer initSequence)
		...
		if (initSequence == null) {
			Integer maxSequence = exerciseQuestionRepo.find("$TaskGetMaxSequence",
					Params.param("exerciseId", exerciseId)).get(Integer.class);
			if (maxSequence != null) {
