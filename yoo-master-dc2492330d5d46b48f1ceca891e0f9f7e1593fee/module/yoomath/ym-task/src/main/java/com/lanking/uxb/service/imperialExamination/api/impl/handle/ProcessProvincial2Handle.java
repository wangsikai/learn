 ...
 */
@Component
public class ProcessProvincial2Handle extends AbstractImperialExaminationProcessHandle {
	@Autowired
	...
	public ImperialExaminationProcess getProcess() {
		return ImperialExaminationProcess.PROCESS_PROVINCIAL2;
	}
	
	public boolean isStatusChanged(long code) {
		ImperialExaminationActivityProcessLog log = activityProcessLogService.get(code, getProcess(),PROVINCIAL_STATUS_CHANGED);
		if (log != null) {
			return true;
		} 
	...
	public synchronized void process(long code) {
		...
		if(!isStatusChanged(code)){
			...
			activityLog.setProcessData(PROVINCIAL_STATUS_CHANGED);
			...
			try {
				...
				logger.error("PROCESS_PROVINCIAL2 fail:"+e);
