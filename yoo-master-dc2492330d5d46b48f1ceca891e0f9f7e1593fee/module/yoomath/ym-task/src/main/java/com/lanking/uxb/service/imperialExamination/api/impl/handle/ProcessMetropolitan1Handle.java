...
/**
 * 会试准备阶段
 * 定时查询老师和学生的乡试成绩并排名，这个阶段可能会有首次考试、冲刺1、冲刺2的成绩
 * */
@Component
public class ProcessMetropolitan1Handle extends AbstractImperialExaminationProcessHandle {
	...
	public ImperialExaminationProcess getProcess() {
		return ImperialExaminationProcess.PROCESS_METROPOLITAN1;
	}
	
	@Override
	public boolean isPublished(long code) {
		ImperialExaminationActivityProcessLog log = activityProcessLogService.get(code, getProcess(),METROPOLITAN_PUBLISHED);
		if (log != null) {
			return true;
		} 
	...
	public synchronized void process(long code) {
		...
		if(!isPublished(code)){
			...
			List<ImperialExaminationQuestion> questions = questionService.get(code,
					ImperialExaminationType.METROPOLITAN_EXAMINATION,null,null,ImperialExaminationExamTag.FIRST_EXAM);
			Date startTime = null;
			Date endTime = null;
			...
			if (questions.size() == 0) {
				logger.error("METROPOLITAN_EXAMINATION FIRST EXAM QUESTIONS SIZE IS 0 ！");
				return;
			}
			...
			for(int room = 1; room < 3; room++){
				if(room == 1){
					...
					for(int textbookCode : firstTextbookCodes){
						questions = questionService.get(code,ImperialExaminationType.METROPOLITAN_EXAMINATION,
								                        room,textbookCode,ImperialExaminationExamTag.FIRST_EXAM);
						
						if (questions.size() == 0) {
							logger.info("METROPOLITAN_EXAMINATION CATEGORY" + textbookCode + "FIRST EXAM QUESTIONS SIZE IS 0 ！");
							continue;
						}
			...
			activityLog.setProcessData(METROPOLITAN_PUBLISHED);
			...
			try {
				...
				logger.error("PROCESS_METROPOLITAN1 fail:"+e);
			...
			for (ImperialExaminationActivityUser user : activtyUser) {
				for (Long clazzId : user.getClassList()) {
					...
					Homework hk = homeworkService.publish(user.getUserId(), clazzId, startTime.getTime(),
							endTime.getTime(), questionIds, "科举大典-会试试题");
					//给老师保存作业
	...
	private void generateTeaAndStuHomework(long code, Integer room, Integer category,
		...
		imperialExaminationHomework.setType(ImperialExaminationType.METROPOLITAN_EXAMINATION);
		...
		if (students.size() == 0) {
			logger.info("METROPOLITAN_EXAMINATION CLASS " + clazzId + " STUDENT SIZE IS 0 ！");
			return;
		}
		...
		for(Student student : students) {
			...
			imperialExaminationHomeworkStudent.setType(ImperialExaminationType.METROPOLITAN_EXAMINATION);
