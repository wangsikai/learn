 ...
 */
@Component
public class ProcessProvincial4Handle extends AbstractImperialExaminationProcessHandle {
	...
	public ImperialExaminationProcess getProcess() {
		return ImperialExaminationProcess.PROCESS_PROVINCIAL4;
	}
	...
	public synchronized void process(long code) {
		...
		if (!isLotteryProcessed(code)) {
			...
			for (ImperialExaminationProcessTime ptime : timeList) {
				...
				// 批改下发时间
				if (ptime.getProcess() == ImperialExaminationProcess.PROCESS_PROVINCIAL3) {
			...
			List<Long> userIds = homeworkService.getIssueHomeworkUserId(code, issueStartTime, issueEndTime,
					ImperialExaminationProcess.PROCESS_PROVINCIAL5);

			if (CollectionUtils.isNotEmpty(userIds)) {
				for (Long id : userIds) {
					lotteryService.addLottery(id, ImperialExaminationProcess.PROCESS_PROVINCIAL5, code);
				}
			}
			...
			activityLog.setProcessData(PROVINCIAL_LOTTERY_PROCESSED);
		...
		try {
			...
			logger.error("PROCESS_PROVINCIAL4 fail:"+e);
