 ...
 * */
@Component
public class ProcessFinalExamination2Handle extends AbstractImperialExaminationProcessHandle {
	@Autowired
	...
	public ImperialExaminationProcess getProcess() {
		return ImperialExaminationProcess.PROCESS_FINAL_IMPERIAL2;
	}
	
	public boolean isStatusChanged(long code) {
		ImperialExaminationActivityProcessLog log = activityProcessLogService.get(code, getProcess(),FINAL_STATUS_CHANGED);
		if (log != null) {
			return true;
		} 
	...
	public synchronized void process(long code) {
		...
		if(!isStatusChanged(code)){
			...
			activityLog.setProcessData(FINAL_STATUS_CHANGED);
			...
			try {
				...
				logger.error("PROCESS_FINAL_EXAMINATION2 fail:"+e);
