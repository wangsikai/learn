...
/**
 * 每日任务： 完成每日练 code == 101010004
 *
 * @author xinyu.zhou
 * @since 4.1.0
 */
@Component
public class DailyPractiseTaskProcessor extends AbstractUserTaskProcessor {
	@Autowired
	...
	public void process(UserTask userTask, long userId, Map<String, Object> params) {
		...
		if (log == null) {
			...
			content.put("content", new StringBuffer().append(userTask.getType().getName()).append(":")
					.append(userTask.getName()).toString());
			...
			updateForm.setStatus(isClient ? UserTaskLogStatus.COMPLETE : UserTaskLogStatus.RECEIVE);
