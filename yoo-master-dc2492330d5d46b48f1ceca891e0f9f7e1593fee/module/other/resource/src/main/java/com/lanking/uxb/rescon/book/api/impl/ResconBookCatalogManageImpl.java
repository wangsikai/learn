...
@Service("bookCatalogManage")
@Transactional(readOnly = true)
public class ResconBookCatalogManageImpl implements ResconBookCatalogManage {
	@Autowired
	...
	public void delete(long id) {
		...
		if (p != null) {
			...
			// 删除章节目录关联
			resconBookCatalogSectionManage.deleteCatalogRelation(p.getBookVersionId(), childrenIds);
		}
	}
	...
	public BookCatalog create(long id, long bookVersionId, String name, long updateId) {
		...
		if (p == null) {
			...
			bookcatalogRepo.save(np);
			bookcatalogRepo.execute("$updateSequence",
					Params.param("bookVersionId", bookVersionId).put("noself", np.getId())
							.put("sequence", np.getSequence()).put("updateId", updateId).put("nowDate", new Date()));
		}
	...
	public void upLevel(long id, long updateId) {
		...
		if (lastP != null) {
			...
			// 将同层上一个目录关联的所有教材章节去除
			resconBookCatalogSectionManage.deleteCatalogRelation(lastP.getBookVersionId(), lastP.getId());
		}
	}
	...
	public void downLevel(long id, long updateId) {
		...
		if (p.getPid() != null && p.getPid() != 0) {
			...
			bookcatalogRepo.save(p);
			bookcatalogRepo.execute("$updateSequence",
					Params.param("bookVersionId", p.getBookVersionId()).put("noself", p.getId())
	...
	public Map<Long, BookCatalog> saveCopy(long bookVersionId, Collection<BookCatalog> catalogs, long createId,
		...
		for (BookCatalog bookCatalog : catalogs) {
			...
			BookCatalogSection oldBookCatalogSection = bookCatalogSectionMap.get(oldId);
			...
			if (oldBookCatalogSection != null) {
				// 需要保存新的章节对应关系
				oldBookCatalogSection.setId(null);
				oldBookCatalogSection.setBookVersionId(bookVersionId);
				oldBookCatalogSection.setBookCatalogId(bookCatalog.getId());
				oldBookCatalogSection.setCreateId(createId);
				oldBookCatalogSection.setCreateAt(date);
				bookCatalogSectionRepo.save(oldBookCatalogSection);
			}
		}
