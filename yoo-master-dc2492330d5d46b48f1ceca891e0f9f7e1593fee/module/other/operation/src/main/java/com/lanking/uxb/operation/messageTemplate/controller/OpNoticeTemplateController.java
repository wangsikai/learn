 ...
 */
@RestController
@RequestMapping(value = "op/messageTemplate/notice")
public class OpNoticeTemplateController {

	@Autowired
	private OpNoticeTemplateService noticeTemplateService;
	@Autowired
	private MqSender mqSender;

	@RequestMapping(value = "listAll", method = { RequestMethod.GET, RequestMethod.POST })
	public Value listAll() {
		List<NoticeTemplate> items = noticeTemplateService.findAll();
	...
	public Value create(String title, String body, String note) {
		NoticeTemplate template = noticeTemplateService.create(title, body, note);
	...
	public Value update(int code, String title, String body, String note) {
		NoticeTemplate template = noticeTemplateService.update(code, title, body, note);
		ValueMap vm = ValueMap.value("template", template);
		return new Value(vm);
	}
	...
	public Value takeEffect() {
		...
		object.put("messageType", MessageType.NOTICE);
